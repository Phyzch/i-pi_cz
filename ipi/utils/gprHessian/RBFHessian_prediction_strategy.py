...
class RBFHessianPredictionStrategy(DefaultPredictionStrategy):
    ...
    def __init__(
        ...
        mvn = self.likelihood(
            train_prior_dist, M, training_data_hessian_data_point_index
        )
    ...
    def covar_cache(self, singular_value_cutoff = pow(10.0, -10)):
        """
    ...
    def exact_predictive_covar(
        ...
        if settings.fast_pred_var.off():
            ...
            covar_correction_rhs = train_train_covar.solve(
                train_test_covar
                ...
                if test_test_covar.dim() == 2:
                    return to_linear_operator(
                        torch.addmm(
                            ...
                            beta=1,
                            alpha=-1,
                        )  # k(x*, x*) - K(x*, X) * covar_correction_rhs
                    )
                else:
                    return to_linear_operator(
                        test_test_covar
    ...
    def exact_prediction(
        ...
        MH_1 = len(
            self.training_data_hessian_data_point_index
        )  # number of training data that contains hessian information.
        MH_2 = len(
            test_data_hessian_data_point_index
        )  # number of test data that contains hessian information.
