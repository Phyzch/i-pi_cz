...
class MAPNEBGPRMover(Motion):
    ...
    def update_GPR_model_one_bead_subroutine(self, training_x, bead_index_for_update, training_bead_forces):
        ...
        self.gpr_model.update_model_with_new_data(training_x, ab_initio_beads_shifted_energy, ab_initio_beads_grad)
        self.nebgm.gpr_model = self.gpr_model
    ...
    def update_GPR_model_with_beads_cause_early_stop(self,  outrange_bead_index_list):
        ...
        gpr_force_amplitude = np.linalg.norm(gpr_grad_x, axis= 1)
        # the prediction of force for beads with small force amplitude is likely to be inaccurate, we do not consider these
        bead_force_amplitude_cutoff = np.max(gpr_force_amplitude) / 3

        # minimum value for allowed trust region ratio. 
        # This is to prevent the algorithm making the trust region ratio too small. 
        # In this case, we will waste time doing unncessary ab initio calculation.
        minimum_trust_region_ratio = 0.005
        ...
        for i in range(len(outrange_bead_index_list)):
            ...
            # we do not consider force whose amplitude is very small. the GPR prediction for the force with small amplitude is prone to have large error.
            if self.ab_initio_force_amplitude_list[i] > bead_force_amplitude_cutoff:
                ...
                if outrange_bead_distance < distance_cutoff and bead_force_diff_ratio > force_error_ratio_cutoff:
                    if self.optarrays["gpr_trust_region_ratio"]  > minimum_trust_region_ratio * 2:
    ...
    def update_GPR_model_one_image_strategy(self, step):
        ...
        beads_forces_amplitude = np.linalg.norm(beads_forces, axis= 1)
        small_value_beads_forces_cutoff = np.max(beads_forces_amplitude) / 5
        ...
        if force_diff_ratio < self.optarrays["gpr_force_criterion"]:
            ...
            while(len(self.bead_index_with_converged_gpr_force) < self.beads.nbeads):
                ...
                if (force_diff_ratio < self.optarrays["gpr_force_criterion"] or 
                    beads_forces_amplitude[bead_index_for_update] < small_value_beads_forces_cutoff):
    ...
    def update_GPR_model_all_image_strategy(self,step):
        ...
        self.gpr_model.update_model_with_new_data(training_x, ab_initio_shifted_energy, ab_initio_grad_x)
        self.nebgm.gpr_model = self.gpr_model
        ...
        # when the bead force is too small, we do not require relative error of force to be small.
        small_value_beads_forces_cutoff = np.max(self.ab_initio_force_amplitude_list) / 5 
    ...
    def update_GPR_model(self, early_stop_bool, outrange_bead_index_list, step):
        ...
        '''
        print("The trust region ratio now: " + str(self.optarrays["gpr_trust_region_ratio"]))
        if early_stop_bool:
