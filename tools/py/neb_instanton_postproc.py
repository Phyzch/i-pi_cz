...
def get_proj_info(q, natoms, nbeads, m, m3, asr, mofi= False):
    """
    ...
    if mofi:
        if asr == "poly":
            return np.prod(I), proj_info
        else:
            return 1.0, proj_info
    else:
        return proj_info
...
def compute_instanton_rate_or_splitting():
    ...
    if not quiet:
        ...
        if exact_diag_bool:
            ...
            hess_logdet_estimate = compute_hessian_logdet(bead_hessian,
                                                          spring_term_param,
                                                          proj_info, 
                                                          pos,
