...
class RBFHessianGaussianLikelihood(_GaussianLikelihoodBase):
    ...
    def __init__(
        ...
        if not has_covar_factor:
            ...
            if grad_covar_factor_rank == 0 or hessian_covar_factor_rank == 0:
                raise (
                    "Must provides the rank of covariance factor for gradient noise & hessian noise if the noise have covariant factor."
                )
            self.grad_covar_factor_rank = grad_covar_factor_rank
            self.hessian_covar_factor_rank = hessian_covar_factor_rank
            self.rank = (
                1 + grad_covar_factor_rank + hessian_covar_factor_rank
            )  # total rank (number of columns) for noise matrix of pot + grad + hessian.

            # check the shape of covar_factor_matrix
            self.noise_covar_factor_pot_grad_array = noise_covar_factor_pot_grad_array
            self.noise_covar_factor_with_hessian_array = (
            ...
            if len(noise_covar_factor_pot_grad_array) != 0:
                ...
                assert (
                    row_size == 1 + ndof
                ), "the number of rows for covar_factor_pot_grad should be the same as the size of gradient in gpr model"
                assert (
                    column_size == 1 + grad_covar_factor_rank
                ), "the number of columns for covar_factor_pot_grad should be the same as the rank of gradient."
            ...
            if len(noise_covar_factor_with_hessian_array) != 0:
                ...
                assert (
                    row_size == 1 + ndof + hessian_triu_size
                ), "the number of rows for covar_factor_with_hessian should be the same as the size of gradient & hessian in gpr model"
                assert (
                    column_size
                    == 1 + grad_covar_factor_rank + hessian_covar_factor_rank
                ), "the number of columns for covar_factor_with_hessian should be the same as the rank of gradient & hessian."
        ...
        if pot_noise_prior is not None:
            self.register_prior(
                "raw_pot_noises_prior", pot_noise_prior, lambda m: m.pot_noises
            )
        ...
        if force_noise_prior is not None:
            self.register_prior(
                "raw_force_noises_prior", force_noise_prior, lambda m: m.force_noises
            )
        ...
        if hessian_noise_prior is not None:
            self.register_prior(
                "raw_hessian_noises_prior",
                hessian_noise_prior,
                lambda m: m.hessian_noises,
            )
    ...
    def pot_noises(self, value: Union[float, Tensor]) -> None:
        self.initialize(
            raw_pot_noises=self.raw_pot_noises_constraint.inverse_transform(value)
        )
    ...
    def force_noises(self, value: Union[float, Tensor]) -> None:
        self.initialize(
            raw_force_noises=self.raw_force_noises_constraint.inverse_transform(value)
        )
    ...
    def hessian_noises(self, value: Union[float, Tensor]) -> None:
        self.initialize(
            raw_hessian_noises=self.raw_hessian_noises_constraint.inverse_transform(
                value
            )
        )

    def update_noise_covar_factor_array(
        self,
        ...
        if len(new_noise_covar_factor_pot_grad_array) != 0:
            ...
            assert (
                row_size == 1 + self.ndof
            ), "the number of rows for covar_factor_pot_grad should fit the size of gradient in gpr model"
            assert (
                column_size == 1 + self.grad_covar_factor_rank
            ), "the number of columns for covar_factor_pot_grad should fit the rank of gradient."
        ...
        if len(new_noise_covar_factor_with_hessian_array) != 0:
            ...
            assert (
                row_size == 1 + self.ndof + self.hessian_triu_size
            ), "the number of rows for covar_factor_with_hessian should fit the size of gradient & hessian in gpr model"
            assert (
                column_size
        ...
        self.noise_covar_factor_with_hessian_array = torch.concat(
            [
                self.noise_covar_factor_with_hessian_array,
    ...
    def _shaped_noise_covar(
        self,
        M: int,
        hessian_data_point_index_array: torch.Tensor,
        *params: Any,
        **kwargs: Any
    ):
        """
        ...
        assert (
            len(self.noise_covar_factor_pot_grad_array) == M
        ), "the size of noise_covar_factor_pot_grad_array is not right. Do you forget to update it when adding new data?"
        assert (
            len(self.noise_covar_factor_with_hessian_array) == M_H
        ), "the size of noise_covar_factor_with_hessian_array is not right. Do you forget to update it when adding new data?"

        if not self.has_covar_factor:
            # The noise matrix is diagonal.
            pot_noises_var = self.pot_noises.repeat(
                [*([1] * n_batch_dim), M]
            )  # shape: [M]
            force_noises_var = self.force_noises.repeat(
                [*([1] * n_batch_dim), M * self.ndof]
            )  # shape: [M * d]
            hessian_noises_var = self.hessian_noises.repeat(
                [*([1] * n_batch_dim), M_H * self.hessian_triu_size]
            )  # shape: [M_H * hessian_triu_size]
            noises_var = torch.concat(
            ...
            pot_noises_std = torch.sqrt(self.pot_noises).type(
                self.noise_covar_factor_pot_grad_array.dtype
            )  # shape [1]
            force_noises_std = (
                torch.sqrt(self.force_noises)
                .repeat([self.grad_covar_factor_rank])
                .type(self.noise_covar_factor_pot_grad_array.dtype)
            )  # shape: [grad_rank]
            hessian_noises_std = (
                torch.sqrt(self.hessian_noises)
                .repeat([self.hessian_covar_factor_rank])
                .type(self.noise_covar_factor_pot_grad_array.dtype)
            )  # shape: [hessian_rank]

            matrix_size = (
                M + M * self.ndof + M_H * self.hessian_triu_size
            )  # the size of covariance matrix.  It is also number of rows for covar_factor
            covar_factor_rank_size = (
                M
                + M * self.grad_covar_factor_rank
                + M_H * self.hessian_covar_factor_rank
            ...
            noise_covar_factor_weight_pot_grad = noise_covar_factor_weight[
                : 1 + self.grad_covar_factor_rank, : 1 + self.grad_covar_factor_rank
            ]

            hessian_data_point_index = -1
            for data_point_index in range(M):
                pot_row_index = np.array([data_point_index])
                grad_row_index = np.arange(0, self.ndof) + (
                    M + data_point_index * self.ndof
                )

                pot_column_index = np.array([data_point_index])
                grad_column_index = np.arange(0, self.grad_covar_factor_rank) + (
                    M + data_point_index * self.grad_covar_factor_rank
                )

                if data_point_index in hessian_data_point_index_array:
                    # the data point index in the hessian_data_point_index_array.
                    hessian_data_point_index = int(
                        torch.argwhere(
                            data_point_index == hessian_data_point_index_array
                        )[0][0]
                    )
                    # the row and column index in noise_covar_factor matrix that corresponds to single data point.
                    hessian_row_index = np.arange(0, self.hessian_triu_size) + (
                        M * (self.ndof + 1)
                        + hessian_data_point_index * self.hessian_triu_size
                    )
                    row_index = np.concatenate(
                        [pot_row_index, grad_row_index, hessian_row_index]
                    )

                    hessian_column_index = np.arange(
                        0, self.hessian_covar_factor_rank
                    ) + (
                        M * (1 + self.grad_covar_factor_rank)
                        + hessian_data_point_index * self.hessian_covar_factor_rank
                    )
                    ...
                    noise_covar_factor_all_data[
                        two_dimensional_index[0], two_dimensional_index[1]
                    ] = torch.matmul(noise_covar_factor, noise_covar_factor_weight)
                else:
                    # the row and column index in noise_covar_factor matrix that corresponds to single data point.
                    ...
                    noise_covar_factor_all_data[
                        ...
                        noise_covar_factor_pot_grad, noise_covar_factor_weight_pot_grad
                    )

            # the covariance matrix of noise is covar_factor @ (covar_factor)^T
            noise_covar_matrix = torch.matmul(
                noise_covar_factor_all_data,
...
def power_iteration(mat: gpytorch.lazy.LazyEvaluatedKernelTensor, num_iters=20):
    ...
    """
    n = mat.shape[-1]
    b_k = torch.randn(n, device=mat.device, dtype= torch.float64)
